...
def extract_article(url: str) -> dict:
    ...
    if not text or len(text) < 200:
        logger.info("Trafilatura short/fail → newspaper3k")
    ...
    if not text or len(text) < 200:
        logger.info("newspaper3k short/fail → readability")
        doc = Document(html)
    ...
    logger.info("Extractor=%s | Title=%r | Author=%r | Words=%d", source, title, author, wc)
    return {
        "text": text,
        "title": title,
        "author": author,
...
def submit_url():
    ...
    try:
        tts_uri = synthesize_long_text(meta["text"], meta["title"], meta["author"], item_id)
